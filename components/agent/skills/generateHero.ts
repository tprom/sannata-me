...
type GenerateHeroResult = {
  type: "success" | "error";
  message: string;
  data?: { city: string; landmark: string; hero: string };
};

// Skill v1.6: обновляет поле hero и регистрирует hero-изображение для достопримечательности.
export async function generateHero(
  ...
  const heroDir = path.join(landmarkDir, "hero");
  const dataPath = path.join(landmarkDir, "data.json");

  // Создаём папку hero, если её нет.
  ...
  // Обновляем только поле hero.
  data.hero = `hero/${heroImage}`;
