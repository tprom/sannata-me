...
type NormalizeLandmarkResult = {
  type: "success" | "error";
  message: string;
  data?: { city: string; landmark: string; normalized: boolean };
};

// Skill v1.3: нормализует структуру данных достопримечательности.
export async function normalizeLandmark(
  cityName: string,
  landmarkName: string
): Promise<NormalizeLandmarkResult> {
  ...
  if (data.title === undefined) {
    data.title = target.title ?? landmarkName;
  }
  ...
  return {
    ...
    data: { city: cityName, landmark: landmarkName, normalized: true },
  };
}
