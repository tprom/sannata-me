...
type GenerateGalleryResult = {
  type: "success" | "error";
  message: string;
  data?: { city: string; landmark: string; added: string[] };
};

// Skill v1.4: обновляет массив images для галереи достопримечательности.
export async function generateGallery(
  ...
  // Создаём папку gallery, если её нет.
  ...
  for (const image of images) {
    (data.images as string[]).push(image);
  }
